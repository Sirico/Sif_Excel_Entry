...
form_submission = """  # write form data to an excel file, for every size create a new row in the excel file
        ...
        parent_dir = 'flaskblog/static/Exports'
        ...
        xlsx = f'SKU:{SKU}-User:{User}- time:{time}- Date:{date}.xlsx'
        csv = f'SKU:{SKU}-User:{User}- time:{time}- Date:{date}.csv'
        ...
        if form.Finalise.data == True:
            ...
            shutil.move(f'{form.PurchaseOrder.data}:{date} Finalised.xlsx',
                        'flaskblog/static/Exports/Finalised')
            shutil.move(f'{form.PurchaseOrder.data}:{date} Finalised.csv',
                        'flaskblog/static/Exports/Finalised')
            flash('File finalised and moved successfully')
"""
...
def adults_footwear():
    ...
    if request.method == 'POST':form_submission
